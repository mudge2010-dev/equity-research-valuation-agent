...
def build_excel(
    ...
    outputs = [
        ...
        ("PG-Implied Intrinsic Value/share", "=B44/$B$17"),
        ...
        ("Exit Multiple-Implied Intrinsic Value/share", "=B50/$B$17"),
...
with tabs[0]:
    ...
    st.markdown("#### DCF implied intrinsic value ranges")
    company_dcf_placeholder = st.empty()
    company_dcf_placeholder.info(
        "Open/review the DCF-WACC Calculation tab to calculate the Perpetuity Growth and "
        "Terminal EBITDA intrinsic-value ranges."
    )
...
with tabs[2]:
    ...
    if not gate_passed():
        ...
        st.markdown(
            "**Student task:** choose 5–7 ratios that matter economically for the company. "
            "Explain the trend, then use the 10-K to investigate the cause."
        )
...
with tabs[5]:
    ...
    if not gate_passed():
        ...
        if dcf_ready:
            ...
            o1.metric("PG-Implied Intrinsic Value/share", f"${dcf_out['PG Implied Share Price']:,.2f}")
            o2.metric("Exit Multiple-Implied Intrinsic Value/share", f"${dcf_out['Exit Implied Share Price']:,.2f}")
            ...
            st.markdown("#### Sensitivity analysis assumptions")
            sensitivity_profile = st.selectbox(
                "Company type for terminal-value sensitivity ranges",
                [
                    "Mature / low-growth",
                    "General / stable",
                    "Growth",
                ...
                index=1,
                help=(
                    "This classroom profile controls the range of perpetuity growth rates and exit EV/EBITDA "
                    "multiples tested. Students should select the profile that best fits the company and justify it."
                ),
            )
            profile_ranges = {
                "Mature / low-growth": {"g": (0.01, 0.03), "m": (5.0, 9.0)},
                "General / stable": {"g": (0.01, 0.05), "m": (6.0, 12.0)},
                "Growth": {"g": (0.02, 0.06), "m": (8.0, 16.0)},
                "Cyclical / asset-heavy": {"g": (0.00, 0.03), "m": (4.0, 10.0)},
            }
            selected_range = profile_ranges[sensitivity_profile]

            # More granular grids than the original 5x5 tables.
            wacc_values = np.linspace(max(0.01, wacc - 0.03), wacc + 0.03, 9)
            g_values = np.linspace(selected_range["g"][0], selected_range["g"][1], 9)
            multiples = np.linspace(selected_range["m"][0], selected_range["m"][1], 9)
            ...
            st.caption(
                f"{sensitivity_profile}: perpetuity growth range {g_values.min():.1%}–{g_values.max():.1%}. "
                "Lower WACC generally increases value; higher terminal growth generally increases value."
            )
            ...
            st.caption(
                f"{sensitivity_profile}: exit EV/EBITDA range {multiples.min():.1f}x–{multiples.max():.1f}x."
            )

            # Football-field ranges hold WACC at the selected base-case WACC and vary only the
            # terminal-growth rate or exit multiple. This makes the range labels economically transparent.
            pg_range_values = []
            for g_test in g_values:
                if wacc > g_test:
                    _, range_out = build_dcf(
                        base_revenue, growths, ebit_margin, tax_rate, da_rev, capex_rev,
                        nwc_rev, wacc, g_test, exit_multiple, net_debt, diluted_shares
                    )
                    pg_range_values.append(range_out["PG Implied Share Price"])

            exit_range_values = []
            for m_test in multiples:
                _, range_out = build_dcf(
                    base_revenue, growths, ebit_margin, tax_rate, da_rev, capex_rev,
                    nwc_rev, wacc, terminal_growth, m_test, net_debt, diluted_shares
                )
                exit_range_values.append(range_out["Exit Implied Share Price"])

            pg_low = float(np.nanmin(pg_range_values)) if pg_range_values else np.nan
            pg_high = float(np.nanmax(pg_range_values)) if pg_range_values else np.nan
            exit_low = float(np.nanmin(exit_range_values)) if exit_range_values else np.nan
            exit_high = float(np.nanmax(exit_range_values)) if exit_range_values else np.nan

            # Populate the Company tab with the same DCF ranges calculated here.
            company_dcf_placeholder.markdown(
                f"**Perpetuity Growth approach:** ${pg_low:,.2f}–${pg_high:,.2f} per share  \n"
                f"**Terminal EBITDA / Exit Multiple approach:** ${exit_low:,.2f}–${exit_high:,.2f} per share  \n"
                f"*Sensitivity profile: {sensitivity_profile}; WACC held at {wacc:.2%} for these displayed ranges.*"
            )

            st.markdown("#### DCF valuation summary")
            chart_ranges = [
                (
                    f"DCF Value at {multiples.min():.1f}x-{multiples.max():.1f}x Exit\nEBITDA Range",
                    exit_low, exit_high, "#F07C2B"
                ),
                (
                    f"DCF Value at {g_values.min():.1%}-{g_values.max():.1%}\nPerpetuity Range",
                    pg_low, pg_high, "#FF0000"
                ),
            ]
            low_52 = market.get("fiftyTwoWeekLow")
            high_52 = market.get("fiftyTwoWeekHigh")
            if low_52 is not None and high_52 is not None and pd.notna(low_52) and pd.notna(high_52):
                chart_ranges.append(("52 Week Market High/Low", float(low_52), float(high_52), "#00AF4F"))

            valid_lows = [x[1] for x in chart_ranges if pd.notna(x[1])]
            valid_highs = [x[2] for x in chart_ranges if pd.notna(x[2])]
            if pd.notna(px):
                valid_lows.append(float(px))
                valid_highs.append(float(px))

            if valid_lows and valid_highs:
                ymin = min(valid_lows)
                ymax = max(valid_highs)
                pad = max((ymax - ymin) * 0.18, 5.0)
                fig, ax = plt.subplots(figsize=(10, 5.2))
                bar_width = 0.42
                for x, (label, lo, hi, color) in enumerate(chart_ranges):
                    ax.bar(x, hi - lo, bottom=lo, width=bar_width, color=color)
                    ax.text(x, hi + pad * 0.12, f"{hi:,.2f}", ha="center", va="bottom", fontsize=9)
                    ax.text(x, lo - pad * 0.12, f"{lo:,.2f}", ha="center", va="top", fontsize=9)
                if pd.notna(px):
                    ax.axhline(float(px), color="red", linewidth=1.0)
                    ax.text(-0.48, float(px), f"{float(px):,.2f}", color="red", va="center", ha="right", fontsize=9)
                ax.set_xticks(range(len(chart_ranges)))
                ax.set_xticklabels([x[0] for x in chart_ranges], fontsize=9)
                ax.set_ylabel("Implied intrinsic value / market price ($ per share)")
                ax.set_title("DCF Valuation Summary", fontweight="bold")
...
with tabs[7]:
    ...
    if not gate_passed():
        ...
        if not dcf_inputs_ss or not dcf_outputs_ss:
            ...
            st.download_button(
                "Download memo prompt",
                data=prompt.encode("utf-8"),
                file_name=f"{ticker}_equity_research_memo_prompt.txt",
                mime="text/plain",
            )
